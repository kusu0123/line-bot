...
def handle_message(event):
    ...
    reply_message = TextSendMessage(text="メッセージをありがとう！\n「〜を調べる」または「〜を忘れた」と話しかけてみてね。") 
    if "を調べる" in message_text:
        ...
        if not item_name_to_search:
            ...
            if found_items_info:
                ...
                # 検索結果がなかった場合
                reply_message = TextSendMessage(text=f"'{item_name_to_search}' の忘れ物は見つかりませんでした。")
        ...
        if not package_name_to_add:
            ...
            supabase.insert_package(PSYCOPG_URL, package_name_to_add)
